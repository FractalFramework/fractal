...
if(!$ex)sqlsav('images',[ses('uid'),substr($f,0,-4),$f,$src,3]);}
...
$u=$p['urlmv']; $hk=val($p,'hk');
$f=strid($u).ext($u);
$root='disk/usr/'.ses('user').'/video/'.$f;
$d=get_file($u); if($d)write_file($root,$d);
self::add_img_catalog($f,'mov');
if($hk)$f='['.ses('user').'/video/'.$f.':video]';
return $f;}
...
$rid='upfile'.$p['rid']??''; $ty=val($p,'ty');
if(isset($_FILES[$rid]))return fsize($_FILES[$rid]['tmp_name']);
else return 'uploading...';}

static function save($p){$error=''; $rid='upfile'.($p['rid']??''); $ty=val($p,'ty'); 
$f=$_FILES[$rid]['name']; $f_tmp=$_FILES[$rid]['tmp_name']; //pr($_FILES);
if(!$f)return 'no file uploaded ';
$xt=ext($f); $f=normalize(struntil($f,'.'));
$goodxt='.mp4.m4a.mov.mpg.mp3.wav.wmv.swf.flv.jpg.png.gif.pdf.txt.xls.csv.json.docx.7z.zip.tar.gz.mid.xhtml.html';
...
$r=sql('img','images','rv','where uid="'.ses('uid').'" order by up desc'); $o=val($p,'o');
...
$ret.='<form id="upl'.$rid.'" action="" method="POST" onchange="upload(\''.$rid.'\',\''.ses('user').'\')"><label class="uplabel btn"><input type="file" id="upfile'.$rid.'" name="upfile'.$rid.'" multiple />'.ico('upload').'</label></form><span id="'.$rid.'up"></span>';
return $ret;}
