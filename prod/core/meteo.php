...
static $towns=['75101'=>'Paris','92012'=>'Boulogne-Billancourt','35238'=>'Rennes','59350'=>'Lille','67482'=>'Strasbourg','69381'=>'Lyon','06088'=>'Nice','13210'=>'Marseille','34172'=>'Montpellier','66136'=>'Perpignan','31555'=>'Toulouse','64102'=>'Bayonne','33063'=>'Bordeaux'];//,'29019'=>'Brest','2B033'=>'Bastia','44109'=>'Nantes','63113'=>'Clermont-Ferrand'
...
if(!$d){$d=@file_get_contents($u); if($d)$_SESSION['d'.$day][$u]=$d;}
...
$nb=1;//Nuit=0; Matin=1; Après-midi=2; Soir=3
...
//$vr['insee']='92012';
$n=$vr['day']??0;//0->13
$nb=$vr['period']??1;//Nuit=0; Matin=1; Après-midi=2; Soir=3
...
$cols=implode(',',self::$cols); //$cols=sqlcls($db,2);
...
$res=sql('res',self::$db,'v',['insee'=>$insee,'day'=>$day,'_limit'=>1],0); $res=str_replace('u0','\u0',$res);
//db::add('usr/dav/meteo/snifer',[day(),ip(),$uid,$insee,$res],['time','ip','uid','insee','res']);
$ra=json_decode($res,true); $tw=$ra['town']??'';
if(!$tw or $x){$vr['insee']=$insee; $vr['qday']=0; $vr['period']=$pe; $vr['radius']=$p['radius']??10;
	...
	if($x){$id=sql('id',self::$db,'v',['insee'=>$insee,'day'=>$day]); if($id && $rb)sqlup(self::$db,'res',$res,$id,'',0);}
	elseif($rb)sqlsav(self::$db,[$uid,$insee,$day,prm($p),$res],0);}
return $res;}
