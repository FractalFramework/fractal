...
if($remain==0)return dechex($last); else return bcdec2hex($remain).dechex($last);}
...
static function numerology($d){$n=strlen($d); $ret=0;
	...
	if(strlen($ret)>1)$ret=numerology($ret);
	return $ret;}
...
function which($d,$rb){foreach($rb as $k=>$v)if(strpos($d,$v))return (int)trim(str_replace($v,'',$d)); return 0;}
foreach($r as $k=>$v)$rc[$k]=which($v,$rb); 
foreach($rc as $k=>$v)if($v){$u=$rb[$k]; $$u=$v;}
$res=mktime2($yr,$mt+1,$dy+1,$hr,$min,$sec);
$mil=mktime2(0,1,1,0,0,0);//note the +1
return $res-$mil;}

static function sectime($d){$yr=$mt=$dy=$hr=$mn=$sc=0;
function rmn($d,$n){$a=$d/$n; $b=floor($a); $c=($a-$b)*$n; return [$b,$c];}
if($d>($n=86400*365.2422))[$mt,$d]=rmn($d,$n);
if($d>($n=2629743.84))[$mt,$d]=rmn($d,$n);
if($d>86400)[$dy,$d]=rmn($d,86400);
if($d>3600)[$hr,$d]=rmn($d,3600);
if($d>60)[$mn,$d]=rmn($d,60);
if($d>1)[$sc,$d]=rmn($d,1);
return $yr.'yr '.$mt.'mt '.$dy.'d '.$hr.'h '.$mn.'m '.$sc.'s';}
...
function rmn($d,$n){$a=$d/$n; $b=floor($a); $c=($a-$b)*$n; return [$b,$c];}
if($d>21600)[$tau,$d]=rmn($d,21600);
if($d>360)[$hr,$d]=rmn($d,360);
if($d>60)[$mn,$d]=rmn($d,60);
if($d>1)[$dg,$d]=rmn($d,1);
if($d>1/60)[$am,$d]=rmn($d,1/60);
if($d>1/3600)[$as,$d]=rmn($d,1/3600);
if($d>1/21600)[$mas,$d]=rmn($d,1/21600);
