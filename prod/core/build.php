...
if(!$a)$a='admin_sql'; if(!isset($p['pg']))$p['pg']=1; $pg=$p['pg'];
if(!isset($p['did']))$p['did']=randid('dt'); else $ny=1; $did=$p['did'];
$prm=prm($p); if($jb)$jb=$a.',sav_cell|'.$prm; $i=0; $bt='';
$n=count($r); $nbp=500; $min=($pg-1)*$nbp; $max=$pg*$nbp;
if($r['_']??''){$rh=$r['_'];
	$bk=popup($a.',edit_row|'.prm($p).',k=_','_'); $td=tag('th','',$bk);
...
if(is_array($r))foreach($r as $k=>$v){if($i>=$min && $i<$max){$td='';
	$bk=popup($a.',edit_row|'.prm($p).',k='.$k,$k); $td.=tag('td','',$bk);
...
$s=$p['start']; $e=$p['end']; $txt=val($p,'txt'); $id=$p['id'];
return div($s.'-'.$e,'wyg');}
...
foreach($r as $k=>$v)$ret.=btj($k,insert('['.utf8_encode($v).':'.$k.']',$rid),'btn').' ';
return div($ret);}

static function editconn($p){$id=$p['aid']??''; $txt=val($p,$id); $rid=randid('edc');
...
list($u,$id,$o,$b)=vals($p,['url','id','o','html']); $x=hlpbt('import_art');
...
list($u,$o,$b,$tg,$hk)=vals($p,['url','o','html','tg','hk']); $tg=$tg?$tg:'urlim';
...
list($u,$o,$b,$tg,$hk)=vals($p,['url','o','html','tg','hk']); $tg=$tg?$tg:'urlmv';
$j='html='.$b.',url='.$u.',tg='.$tg.',hk='.$hk; $x=hlpbt('import_mov');
if($o)return inputcall($tg.'|upload,import_mov|'.$j.'|urlmv','urlmv',$u?$u:lang('movie'),22,1).$x;
$bt=toggle($rid.'|build,import_mov|'.$j.',o=1',langpi('import_mov'),'btn');
return $bt.span('','',$rid);}

static function import_db($p){$rid=randid('mpb');
list($u,$id,$cb,$a,$o)=vals($p,['url','id','cb','a','o']);
$x=bj($rid.'|build,import_db',pic('cancel'),'btn').hlpbt('import_db');
if($o)$ret=inputcall($cb.'sub|'.$a.',subops|op=imp,id='.$id.'|impdb','impdb',$u?$u:lang('root'),22).$x;
else $ret=bj($rid.'|build,import_db|o=k,id='.$id.',cb='.$cb.',a='.$a,langpi('import_db'),'btn');
return span($ret,'',$rid);}
...
if($k)$d=$r[$k]??''; if($d)return utf8_encode($d); elseif($k)return;
