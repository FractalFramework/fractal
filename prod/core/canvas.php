...
list($p,$o,$c,$da)=readgen($d); $ret='';
switch($c){
	case('w'):self::$w=$p; return; break;
	case('h'):self::$h=$p; return; break;
	case('dim'):list($w,$h)=expl('/',$p); self::$w=$w; self::$h=$h; return; break;
...
list($w,$h,$t2)=expl(',',$o,3); if($w)self::$w=$w; if($h)self::$h=$h; if($t2)$t=$t2;
$js=self::read($d,$o);
...
$ret.=jscode($js,'jsc'); //$ret.=hidden('jsc',addslashes($js));
