...
$min=min($ra); $max=max($ra); $diff=$max-$min; $ratio=1/$diff;
self::$ratios[$i]=$ratio; self::$incs[$i]=$diff/($n); self::$mins[$i]=$min;
foreach($r0 as $k=>$v)$ry[$k][$i]=($v[$i]-$min)*$ratio;}
return $ry;}
...
$x=($rx[$i]??$rx[$i-1])*$wb+$bdr[0];
$rot=strlen($t)>4?',,,,,rotate(300 '.($x-2).'/'.($bdr[1]+$h2-10).')':'';
if(isset($rc))$ret.='[black,'.$rot.':attr]['.($x-8).','.($hc).',,*'.$rc[$i].':text]';
$rot=strlen($t)>4?',,,,,rotate(300 '.($x+8).'/'.($y+10).')':'';
...
static function histo($rp,$r0,$bdr,$hb,$h2,$dv,$lb,$clr,$wb){$ret=''; $txt='';
$na=count($rp); $w=$wb/$na; if($w>20)$w=20; $mxy=[]; $w2=$w/2;
$hb=$hb+$h2+$bdr[1];//frame
foreach($rp as $k=>$v){$n=count($v); $old=0; //arsort($v); //pr($v);
	foreach($v as $ka=>$va){
	$x=$va['x']; $hc=$va['y']; $y=$hb-$hc;//
	$ret.='['.$clr[$ka].',black:attr]['.($x-$w2).','.($y-$old).','.($w).','.($hc).':rect]';
	$t=$r0[$k][$ka]; if($dv)$t=numday($t);
...
list($typ,$dk,$dv,$ad,$dc,$pr,$lb,$ti)=vals($p,['typ','dk','dv','ad','dc','pr','lb','t']); //pr($rp);
...
//keys
$rktx=array_keys($r0); //pr($rkt);//sort($rk);
