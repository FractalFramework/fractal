...
if($b=='epub'){$f='usr/_epub/OEBPS/images/'.$d; if(!is_file($f))copy('img/full/'.$d,$f); return img($f);}
...
if(in_array($c,$r)){return tag($c,$atb,$p);}//if($o)$p=self::url($d,'');
$r=['h'=>'big','k'=>'strike','q'=>'blockquote','s'=>'small','e'=>'sup','n'=>'sub','c'=>'center'];
if(isset($r[$c]))return tag($r[$c],'',$d);
if($xt=strend($da,'.')){
...
switch($c){
	...
	case('figure'):return tag('figure','',img($p).tag('figcaption','',$o)); break;
	...
	case('clr'):return span($p,'','','color:#'.$o.';'); break;
	case('bkg'):return span($p,'','','background-color:#'.$o.'; color:#'.clrneg($o,1).';'); break;
	case('cod'):return div(tag('code','',$d),'console'); break;
	case('php'):return build::code($d); break;
...
static function app($c,$p,$t,$b){self::$obj[$c][]=[$p,$t];
	...
	if($op){//conn::$one!=1 && 
		//if($op==2)$ret=app($c,['appMethod'=>'preview','id'=>$p,'ptag'=>'no','opt'=>$b]);
		if($op==2)$ret=$q::preview(['id'=>$p,'ptag'=>'no','opt'=>$b]);
		elseif($op==3)$ret=ifrapp($c,$p);
		//elseif($op==4)$ret=lk(host(1).'/frame/'.$c.'/'.$p,$bt,'app',1);
		elseif($op==4)$ret=lk(host(1).'/'.$a.'/'.$p,$bt,'app',1);
		//else $ret=app($c,['appMethod'=>'call','id'=>$p,'ptag'=>'no']);}//,'conn'=>'no'
		else $ret=$q::call(['id'=>$p,'ptag'=>'no']);}//,'conn'=>'no'
	elseif($t==1)$ret=$q::play($prm);
	//elseif($b==1)$ret=app($c,['appMethod'=>'call','id'=>$p,'ptag'=>'no']);
...
$d=val($p,'msg'); if(!$d)$d=val($p,'p1'); //self::$usd=1;
