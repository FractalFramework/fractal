...
$usr=sqlin('name',$app::$db,'login','uid','v',$id);
return 'usr/'.$usr.'/'.$app.'/'.$id;}

#editable
//cell
static function sav_cell($p){
$f=$p['f']; $id=$p['id']; list($k,$kb)=explode('-',$id); $kb-=1;//fix
$v=val($p,'d'.$id); $v=trim(str_replace('<br>','',$v));
$r=self::read($f); if(isset($r[$k][$kb]))$r[$k][$kb]=$v; self::save($f,$r);
return $v;}
...
if(isset($rk))foreach($rk as $k=>$v)$r[$ka][$k]=trim(stripslashes(str_replace('<br>','',val($p,$rid.$k))));
self::save($f,$r);
return self::nav($p);}

static function edit_row($p){$f=$p['f']; $ka=$p['k']; $pg=$p['pg']??'';
$r=self::read($f); $ra=current($r); $n=count($ra); $ri=val($r,$ka); $rid=randid(); $i=0;
...
if($ra)foreach($ra as $k=>$v)$ret[]=[$v,goodinput($rb[$k],val($ri,$k))];
...
#files
static function del($p){$f=$p['f']; $ok=val($p,'ok');
...
if($f)$rb[]=bj('ftedit|explorer,displace|f='.$f.',nm='.$bck,langpi('back').strfrom($bck,'/'),'cicon grey');
$rb[]=bj('fcb|explorer,opsav|op=rename_file,f='.$f.',nm='.$nm.'/'.strend($f,'/'),langp('save in').' '.strfrom($nm,'/'),'bicon');
if($r)foreach($r as $k=>$v)if(!is_numeric($k))$rb[]=bj('ftedit|explorer,displace|f='.$f.',nm='.$nm.'/'.$v,ico('folder').$v,'cicon');
if(isset($rb))$ret=implode('',$rb);
...
$f=$p['f']; $op=val($p,'op'); $nm=trim(val($p,'nm')); $no=0; $xt=ext($f); $r=[];
if(strpos('.mp3.csv.jpg.png.gif.gz.mp4.xhtml',$xt)===false)$r=self::read($f);
switch($op){
...
case('mov_row'):$r=self::mvrow($r,$nm,val($p,'mv')); break;
...
case('import_sql'):if($nm!='login')$r=sql('all',$nm,'rr',''); array_unshift($r,sqlcols($nm,6)); break;
...
$ret.=bj('ftedit|explorer,imtool|f='.$f,langpi('cancel'),'btn');
return $ret;}

#actions
static function imtool($p){$f=$p['f']; $ret='';
$r=['import_table','import_html','import_conn','import_csv','import_json']; if(auth(6))$r[]='import_sql';
if(isset($r))foreach($r as $k=>$v)$ret.=toggle('ftedit|explorer,opedt|op='.$v.',f='.$f,langph($v),'');
return div($ret,'nbp');}

static function dtool($f){$ret='';
$r=['add_dir','add_file','rename_dir','del_dir'];
...
array_push($r,'add_row','add_col','add_key','renumber','trunc');
if($er){$r=['repair']; $c='del';}
foreach($r as $k=>$v)$ret.=bj('fcb|explorer,opsav|op='.$v.',x='.$x.',f='.$f,langpi($v),$c);
$r=['reorder','del_col','swap','editxt','editcsv','editjson','duplicate','displace','rename_file','del_file'];
if(!$er)foreach($r as $k=>$v)$ret.=toggle('ftedit|explorer,opedt|op='.$v.',x='.$x.',f='.$f,langpi($v),$c);
$ret.=toggle('ftedit|explorer,imtool|f='.$f,langpi('import'),'');
...
#f
static function secu($r){$usr=ses('user');
if($r[0]=='.')$r[0]='usr';
$dr0=['usr','db','json']; if(auth(6))$dr0[]='_';
...
static function url($f,$o=''){$usr=ses('user');
