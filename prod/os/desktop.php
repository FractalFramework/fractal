...
static function headers(){//add_head('jscode','');
...
$cols=$a::$cols??[];
	...
	if($rb)foreach($rb as $kb=>$vb){
		...
		if($t && $a!='images')sqlsavup(self::$db,$pr,['uid'=>$uid,'dir'=>$dir,'com'=>$com]);//pr($pr);
		$rt[]=$pr;}}
...
$ex=sql('dir',self::$db,'v','where dir like "/apps/%" and com="'.$app.'"');
...
return $ret.bj('popup|desktop|dir='.$ex,lang('desktop').$ex,'btn');}
...
$idv='mdf'.$p['id'].$p['col'];
$js=bjs('div,'.$p['cbk'].'|desktop,savemdf|cbk='.$p['cbk'].',id='.$p['id'].',col='.$p['col'].',idv='.$idv.'|'.$idv); $v=val($p,'val');
...
$dir=$p['dir']; $cuid=val($p,'cuid');
$uid=ses('uid'); $usr=ses('user'); $spd=ses('dskspd');//3=usr,2=public,1=private
...
static function pick($p){$css='bicon'; $cuid=ses('uid'); $id=val($p,'id'); $ret='';
...
//self::install();
$uid=val($p,'cuid',ses('cuid')); $dir=val($p,'dir'); if($dir && substr($dir,0,1)!='/')$dir='/'.$dir;
