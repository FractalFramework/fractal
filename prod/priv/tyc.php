...
$dr='usr/'.ses('user').'/tyc/'; $u=$dr.$fa;
//$dr2='disk/usr/'.ses('user').'/csv/tyc/'; if(!is_dir($dr2))mkdir_r($dr2);
$xt=ext($fa);
...
//$dr2='disk/usr/'.ses('user').'/csv/tyc'; //rmdir_r($dr2);
...
if($cols)sqlcreate(self::$db2,$cols,1);
$r=sql($rc,self::$db,'','');
}
