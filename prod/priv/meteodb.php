...
static function meteovls($r){$rb=[]; $na=59;
foreach($r as $k=>$v)if($v){
	foreach($v as $ka=>$va){
		if($ka==1){$va=date('Y-m-d H:i:s',strtotime($va));}
		if($va=='mq' or !$va)$r[$k][$ka]='null';}
...
$dr2='disk/usr/'.ses('user').'/csv/mfr/'; //rmdir_r($dr2);
if($xt=='.gz'){$fa=struntil(strend($fa,'/'),'.'); $ub=$dr2.$fa.'.csv'; echo $ub.' '; if(!is_dir($dr2))mkdir_r($dr2);
	if(!is_file($ub)){$d=readgz($u); write_file($ub,$d);} $u=$ub;
	//$fb='disk/usr/'.ses('user').'/csv/'.$fa; if(is_file($fb))unlink($fb);
	}
$r=readcsv($ub,';'); $cols=[]; //echo count(current($r));//pr($r);
$rk=array_shift($r); //p($rk);
//$cols=self::meteocls($rk); p($cols);
//if($cols)sqlcreate($db,$cols,1);
...
$ra=scandir_b($dr); //pr($ra);
$dr2='disk/usr/'.ses('user').'/'; //rmdir_r($dr2.'csv/mfr'); //rmdir_r($dr2.'archive');
...
$rc=explode(',',$cls); $rt=explode(',',$clt); //$cols=array_combine($rc,$rt); sqlcreate(self::$db2,$cols,1); //pr($cols);
...
$r=validcols($r,self::$db2);
sqlsav2(self::$db2,$r,1);}
...
foreach($r as $k=>$v)sqlup('meteo_paris','weather',$v?$v:'null',$k,'date','');}
