...
$root=ses('site'); //if(!$root)$root='fractal';
$r[]=[$root,'lk','/','','home'];
//if($a)$r[]=[$root,'pop','core,help|ref='.$a.',conn=1',$a,$a];
//$r[]=[$root,'pop','core,help|ref=features,conn=1','art','features'];
//$r[]=[$root,'pop','core,help|ref=fractal,conn=1','fractal','Fractal'];
$r[]=[$root,'pop','core,help|ref=mac_welcome,conn=1,css=article tmpop','mac','welcome'];
$rb=lngs();//['en','es','fr'];
...
//else $r[]=[$root,'pop','applist,showroom','art','applist'];
$r[]=[$root.'/about','pop','core,help|ref=confidentiality,conn=1','info','confidentiality'];
//$r[]=[$root.'/about','pop','multilang,call|id=b2a8c40049','art','developpers'];
//$r[]=[$root.'/about','pop','art,call|id=5','art','developpers'];
//$r[]=[$root,'pop','download','download','Fractal']; //http://logic.ovh/download
...
$r[]=[$root.'/apps','in','loadapp,com|tg=popup','','loadapp'];
//if(auth(2))
$r=applist::menuapp2($r,$root);
//if(auth(2))
//$r[]=[$root.'/apps','in','applist,menuapp','','apps'];
/*$r[]=[$root.'/tlex','pop','art|id=6','tlex','welcome'];
$r[]=[$root.'/tlex','pop','contact','','contact'];*/
if(auth(2))$r[]=[$root.'/about','','tickets','','tickets'];
$r[]=[$root.'/about','pop','devnote','','devnote'];
$r[]=[$root.'/about','pop','contact','','contact'];
//$r[]=[$root,'pop','core,help|ref=credits,conn=1','info','credits'];
$r[]=[$root.'/about','pop','paypal','money','donation'];
$r[]=[$root.'/about','pop','core,help|ref=legals,conn=1','','legals'];
return $r;}

static function logoff($p){
$a=$p['a']; $r=self::about($a);
//$r[]=['','pop','login,com','login','login'];//|auth=2
...
$ret=div(implode('',$rb),'');
if($usr=val($p,'usr'))$ret.=password('psw','').bj('bdg|login|psw',lang('login'),'btsav');//
$ret.=div('','','bdg');
return $ret;}
...
if($dev=='prog'){
	...
	//if(nohttp($_SERVER['HTTP_HOST'])!=serv())
...
$usr=ses('user')?ses('user'):'profile'; //$r[]=['','lk','/'.$a,'',$a];
$r[]=[$usr,'lk','/@'.$usr,'at',$usr];
...
$app=$p['app']??''; ses('app',$app); $own=ses('user');
