...
sqlcreate('cluster_parents',['bid'=>'int','pid'=>'int','ct'=>'int'],1);}
...
foreach($r as $k=>$v)$ret.=span(langp($v),'btn'.act($k,$d));
return $ret;}
...
$ra=pvalk($r,self::$db,1); //pr($ra);
$nid=sqlsav(self::$db,$ra,0);//create new cluster
bank::$rf['new_cluster']=$nid;
$rb=['bid'=>$nid,'cid'=>$bid,'ct'=>$ct];
$kid=sqlsav(self::$db5,$rb);//associate parent
bank::$rf['new_parent']=$kid;
sqlup(self::$db,'status','2',$bid,'',0);//disactivate old cluster
...
if($id)sqldel('cluster_parents',$pid);
//$ct=self::contract($p+['ok'=>0]);
sqlup(bank::$db3,'ok','0',$ct);//resolve contract
sqlup(self::$db,'status','0',$cid,'',0);//if($typ==0)
...
if($cid && $ok!=$ok2)sqlup(bank::$db3,'ok',$ok2,$cid);//surely better to not update
elseif(!$cid && $ok2)$cid=sqlsav(bank::$db3,$r+['ok'=>1]);
sqlup(self::$db,'status',1,$cid);
return $cid;}
...
if($p['pid']??'')sqlup(self::$db,'status','1',$pid);
return self::parents($p);}
...
return btj(langp(''),atj('val',[$id,'pid']).atj('closediv',['adn','this']));}
...
$r=sqlin('cluster_parents.id,pid,tit,ct,value,typ','cluster_parents',self::$db,'pid','id',['bid'=>$id]);
...
if($p['del']??'')sqldel(self::$db2,$id);
else sqlup(self::$db2,'prop',$p['mdfprop'],$id);
...
if(!$ex)sqlsav('cluster_prop',$r);
else sqlups('cluster_prop',$r,$ex);
return self::properties($p);}
...
if($ex)$p['unid']=$ex; else $p['unid']=sqlsav('cluster_unit',['atid'=>$atid,'unit'=>$unit]);
...
if($ex)$p['atid']=$ex; elseif(!$ex_prop)$p['atid']=sqlsav(self::$db3,['attr'=>$attr,'typ'=>$p['typ']]);
...
return sqlrin('cluster_prop.id,attr,prop,unit',[['cluster_attr','id','cluster_prop','atid'],['cluster_prop','unid','cluster_unit','id']],'id',['bid'=>$id]);}
...
$r=sqlrin($db.'.id,tit,typ,value,pid',[[$db,'id','cluster_parents','bid']],'',[]);//'uid'=>$uid,'_group'=>$db.'.id'
...
$r=sqlrin('attr,unit,prop',[[self::$db2,'atid',self::$db3,'id'],[self::$db2,'unid',self::$db4,'id']],'id',['bid'=>$id]);
...
$ret.=tabler($rt);
$n=sqlin('count(pid)','cluster_parents',self::$db,'pid','v',['bid'=>$id]);
...
$bt=ico('code-fork').span($v['tit']); $c='bicon bkg'.self::$credits[$ty];
...
else $r=sqlrout($db.'.id,tit,typ',[['cluster','id','cluster_parents','pid']],'rr',['uid'=>$uid,'!status'=>'3','or'=>['pid'=>'is null'],'_order'=>'id']);//,'>typ'=>0
$bt=bj(self::$cb.'|cluster,stream|mod=1',langp('masters clusters'),'btn'.act($md,'1'));
$bt.=bj(self::$cb.'|cluster,stream|mod=2',langp('all clusters'),'btn'.act($md,'2'));
$bt.=hlpbt('cluster_categories');
$ret=div($bt);
if($r)foreach($r as $k=>$v){$ty=$v['typ'];
	$bt=ico('code-fork').span($v['tit']); $c='bicon ic'.$clr[$ty];
	$ret.=bjk(self::$cb.'|cluster,edit|id='.$v['id'],$bt,$c,$a.'/'.$v['id']);}
return $ret;}
