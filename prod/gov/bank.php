...
sqlcreate(self::$db2,self::$bank_credits,1);
sqlcreate(self::$db3,self::$bank_contracts,1);}
...
if($id)sqlups(self::$db2,$r,$uid,'uid');
else{$r=['uid'=>$uid]+$r; sqlsav(self::$db2,$r,0,0,1);}
...
$r=validcols($r,self::$db3); //pr($r);
$nid=self::bank_trigger($p); self::$rf['trigger']=$nid;
if($nid)$id=sqlsav(self::$db,$r,'','',1); self::$rf['bank']=$r;
...
if(!$er)$er=sqlup(self::$db3,'ok',1,$ct);//&&$id means contract are refs
