<?php

class tetra{	
static $private=2;
static $a=__CLASS__;
static $db='tetra';
static $cols=['tit','txt'];
static $typs=['svar','text'];
static $cb='mdb';

static function install(){
sql::create(self::$db,array_combine(self::$cols,self::$typs),1);}

static function admin(){return admin::app(['a'=>self::$a,'db'=>self::$db,'cb'=>self::$cb]);}
static function js(){return;}
static function headers(){head::add('csscode','');
head::add('jscode',self::js());}

#core
static function aiooya($a,$b){if($a && !$b)return true;}
static function aiooyeedoo($a,$b){if(!$a && $b)return true;}
static function aiooyaammie($a,$b){if($a && $b)return true;}
static function aiooyau($a,$b){if(!$a && !$b)return true;}


#build
static function build($p){$id=$p['id']??'';
$r=sql('all',self::$db,'rr',['uid'=>ses('uid')]);
return $r;}

#play
static function play($p,$r){$ret=''; //pr($r);
if($r)foreach($r as $k=>$v){$rb=[];
	foreach(self::$cols as $kb=>$vb)$rb[]=div($v[$vb]);
	$ret.=div(implode('',$rb));}
return $ret;}

#call
static function call($p){
$r=self::build($p);
$ret=self::play($p,$r);
if(!$ret)return help('no element','txt');
return $ret;}

static function menu($p){
$j=self::$cb.'|'.self::$a.',call||inp2';
$bt=bj($j,langp('ok'),'btn');
//$ret=$bt.textarea('inp2','',60,4);
$ret=inputcall($j,'inp2',$p['p1']??'',32).$bt;
return $ret;}

#content
static function content($p){
//self::install();
$p['p1']=$p['param']??$p['p1']??'';
$bt=self::menu($p);
$ret=self::call($p);
return $bt.div($ret,'pane',self::$cb);}
}
?>