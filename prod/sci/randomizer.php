...
//sqlcreate(self::$db2,['bid'=>'int','uid'=>'int','val'=>'var'],1);//collect case
sqlcreate(self::$db2,['bid'=>'int','nb'=>'int'],1);//subcall case
parent::install(array_combine(self::$cols,self::$typs));}

static function admin($p){$p['o']=1; return parent::admin($p);}
static function titles($p){return parent::titles($p);}

static function injectJs(){$id='';
return '
function batchtime(){
...
static function addrand1($p){sqlsav(self::$db2,[$p['id'],self::generate1($p)]); return self::edit($p);}
static function generate2($p){$ret=''; $n=100; for($i=0;$i<$n;$i++)$r[]=[$p['id'],self::generate1($p)]; return $r;}
static function addrand2($p){sqlsav2(self::$db2,self::generate2($p)); return self::edit($p);}
