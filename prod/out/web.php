...
if(!$r or $x){$ra=$r;// or !$r[0]
	...
	//if($r??[])$r=sql::vrf(array_combine(['tit','txt','img'],$r),'tlex_web');
	$r=array_values($r);
	//if($ra)pr($ra);
	$tit=trim($r[0]);
	$txt=trim($r[1]); if(strlen($txt)>256)$txt=substr($txt,0,256);
	if($ra)sql::upd('tlex_web',['tit'=>$tit,'txt'=>$txt,'img'=>$r[2]],$ra[3]);
	elseif($r[0])sql::sav('tlex_web',[$d,$tit,$txt,$r[2]]);}
return $r?$r:['','',''];}
...
if(conn::$opn)$tit=bj('pagup,,scrollTop|'.$j,span($pic.' '.$t,'apptit'),'appicon');
...
$txt=($r[1]);//utf8_encode
$ret=$tit.div($txt,'stxt').div($url.$bt).div('','',$rid);
