...
//sqlcreate(self::$db,self::$db_cols,1);
//sqlcreate(self::$db3,self::$db3_cols,1);
sqlcreate(self::$db2,self::$db2_cols,1);}

//js to append to the header of the parent page
static function injectJs(){return 'exs=[]';}//reinit the continuous scrolling
...
foreach($ra as $k=>$v){if(isset($rb[$v]))sqldel(self::$db2,$k); $rb[$v]=1;}}

static function twsave($r){
$ex=sql('id',self::$db2,'v','where twid='.$r['twid']);
if(!$ex)sqlsav(twitter::$db2,$r);
else sqlups(self::$db2,$r,$ex);}
...
$date=utf8_encode(strftime('%H:%M - %d %b %Y',$time));
...
if($r)foreach($r as $k=>$v)if(domattr($v,'dir')=='ltr')$ret=utf8_decode($v->nodeValue);
return $ret;}

static function clean($d){//eco($d);
if(strpos($d,'<p')!==false)$d=segment($d,'<p','</p>'); 
...
if($r)foreach($r as $k=>$v){$u=$v['expanded_url'];
	...
	//elseif(substr($u,0,23)=='https://www.youtube.com')$rb[]=segment($u,'v=','&');
...
static function see($p){$id=val($p,'id');
$r=sql('all',self::$db2,'ra',['twid'=>$id]);
return eco($r,1);}

static function usr($p){$id=val($p,'id'); return;
return sqlin('name',self::$db,'login','uid','v',$id);}

static function modif0($p){$id=val($p,'id');
$r=sql('all',self::$db2,'ra',['twid'=>$id]);
...
//admin
static function edit($p){$usr=val($p,'usr');
...
$r=sql('id,owner',self::$db,'kv',['uid'=>ses('uid')]);
if($r)foreach($r as $k=>$v)$ret.=bj('socket|twitter,twusr|twusr='.$v,pic('api').$v,act($c,$k));
if(ses('uid'))$ret.=bj('popup|admin_twitter',langp('params'));
return div($ret,'list');}

static function init($usr=''){$d=2;
$uid=$usr?usrid($usr):ses('uid');
if($usr)list($d,$u)=sql('id,owner',self::$db,'rw',['uid'=>ses('uid'),'owner'=>$usr]);
elseif(!$d=ses('twid'))list($d,$u)=sql('id,owner',self::$db,'rw','where uid="'.$uid.'" limit 1');
