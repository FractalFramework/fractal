...
sqlcreate(self::$db,array_combine(self::$cols,self::$typs),'z');}

static function admin(){return admin::app(['a'=>self::$a,'db'=>self::$db,'cb'=>self::$cb]);}
static function injectJs(){return;}
static function headers(){
add_head('csscode','.orc{position:relative; width:198px; height:330px; font-size:24px; color:yellow; text-shadow:1px 1px 1px rgb(0,0,0,0.7); text-align:center; background-position:center center;
...
57 omega';
$r=explode("\n",$d);
foreach($r as $k=>$v){$rb=explode(' ',$v); sqlsav(self::$db,[utf8_encode($rb[1])]);}}
...
//$im='usr/oracle/'.$in.'.jpg';
//$s='background-image:url('.$im.');';
...
return inputcall($j,'inp2',$p['p1']??'',32).$bt;}
