...
$a=self::$a; $db=self::$db2; $cb=self::$cb; $cols=sqlcls($db,3);
...
$ret.=textarea($rid,'');
$ret.=aj(self::$cb.$id.',,x|sticky,save_note|bid='.$id.',rid='.$rid.',start='.$s.',end='.$e.'|pad,'.$rid,langp('save'),'btsav');
return $ret;}
...
$ret.=aj('socket,,x|sticky,modif_note|id='.$id.'|pad'.$id,langp('modif'),'btsav');
$ret.=aj('socket,,x|sticky,del_notes|del='.$id,langp('del'),'btdel');
return $ret;}

static function stream_notes($p){
$txt=sql('txt',self::$db,'v',$p['id']);
$id=val($p,'id'); $rb[]=[lang('pad'),lang('edit'),lang('delete'),lang('error')];
...
if($r)foreach($r as $k=>$v){$ok=1;
	...
	$sav=aj('popup|sticky,edit_note|id='.$v['id'],picto('edit'),'');
	$del=aj('obso|sticky,del_note|id='.$id.',del='.$v['id'],picto('del'),'');
	$rb[]=[$v['pad'],$sav,$del,$er];}
...
$r=sqlin('name,pad,txt',self::$db2,'login','uid','ra','where '.self::$db2.'.id='.$id); //pr($r);
