...
sqlcreate(self::$db,['vuid'=>'int','cto'=>'int','cmail'=>'var','ctit'=>'var','ctxt'=>'text'],1);}
...
return $ret.' '.bj(val($p,'rid').'|contact,call',langp('back'),'btn');}

//builder
static function read($p){$rid=val($p,'rid');
$r=sqlin('contact.id,name,cto,cmail,ctit,ctxt,dateup',self::$db,'login','vuid','rr','where cto="'.ses('uid').'" order by contact.id desc');
