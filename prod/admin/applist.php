...
$r=self::appdirs();
foreach($r as $dir=>$files){
	if(is_array($files))foreach($files as $k=>$v){
		$f='prog/'.$dir.'/'.$v; if($v==$a.'.php' && is_file($f))return $f;}
	else{$f='prog/'.$dir.'/'.$files; if($v==$a.'.php' && is_file($f))return $f;}}}
...
$ra=self::build('');
if($ra)foreach($ra as $k=>$v)$r[]=[$root.$v,'',$k,'',strend($k,'/')];
return $r;}

static function com($dr=''){$r=self::build($dr);
