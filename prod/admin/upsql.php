...
static function call($p){$a=$p['app'];
$f=serv().'/api/upsql/mth:render,p:'.$a.'';
$d=get_file($f); if($d)$r=json_decode($d,true);
if($_SERVER['HTTP_HOST']!=serv())return self::installdb($a,$r);}
...
if($b)return lastid($b);}

static function whatsnew($p){
$r=explode('-',$p['dbs']); $rt=[];
$ra=query('show tables','rv'); //pr($r);
foreach($r as $k=>$v)if(class_exists($v)){
	if(isset($v::$db)){$b=$v::$db; if(in_array($b,$ra))$rt[]=[$b,lastid($b)];}
	if(isset($v::$db2)){$b=$v::$db2; if(in_array($b,$ra))$rt[]=[$b,lastid($b)];}}
	elseif(in_array($v,$ra) && sqlex($v))$rt[]=[$v,lastid($v)];
return implode_r($rt,';','-');}

#img
static function newim($p){//distant
$ra=scandir_b('img/full'); $rc=[];
...
$r=scandir_b('img/full'); $d=implode("\n",$r); write_file('usr/dl/img.txt',$d);
$f=serv().'/api/upsql/mth:newim,srv:'.host(); $d=get_file($f); $ret=$d.br();
if($d)$d=get_file(serv().'/usr/dl/imb.txt'); $rb=explode("\n",$d);
foreach($rb as $k=>$v)if($v && $k<10)$rc[]=upload::srv_img(serv().'/img/full/'.$v);//
...
$f=serv().'/api/upsql/mth:whatsnew,dbs:'.implode('-',$r); $d=get_file($f);
return explode_r($d,';','-');}

static function batch($p){
$r=self::lastest($p); $ret=[];
foreach($r as $k=>$v){$lid=lastid($v[0]);
...
foreach($r as $k=>$v){$lid=lastid($v[0]); $c=$v[1]>$lid?'btsav':'btn';
