...
sqlcreate(self::$db,['tit'=>'svar','css'=>'json'],1);}
...
foreach($r as $k=>$v){$d=segment($v[0],$a,':before'); if($d)$ret[]=$d;}
return $ret;}
...
foreach($r as $k=>$v){
	$tit=segment($v,"\n",'{'); $tit=exclude($tit,'/*','*/');
	$txt=segment($v,'{','}'); $txt=exclude($txt,'/*','*/'); $txt=deln($txt);
	$rb=explode_k($txt,';',':');
...
$ret=div(batch($r,self::$a.'|css,call|css=$v'),'sticky');
