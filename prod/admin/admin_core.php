...
$ret.=aj('md'.$id.'|admin_core,update|id='.$id.'|tx'.$id,pic('save'));
...
if($r)foreach($r as $k=>$v){$id=$v['id'];
	...
	if(auth(6))$bt.=aj('popup|admin_core,seecode|id='.$id,langp('view'),'btn');
	if(auth(6))$bt.=aj('md'.$id.'|admin_core,modif|id='.$id,langp('modif'),'btn').br().br();
...
self::install();
$rid=randid('dcl');
$bt=aj($rid.'|admin_core,reflush|rid='.$rid,langp('update'),'btn');
$bt.=aj('popup,,xx|core,mkbcp|b=syscore',langp('backup'),'btsav');
$bt.=aj('popup,,xx|core,rsbcp|b=syscore',langp('restore'),'btdel');
