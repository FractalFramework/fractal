...
sqlcreate(self::$db,['ref'=>'var','lg'=>'svar'],1);
sqlcreate(self::$db2,['bid'=>'int','aid'=>'int','app'=>'var'],1);}
...
else sqlsav(self::$db,[$p['ref'],$p['txt'],$p['lg']]);}
...
elseif($txt)$voc=yandex::com(['from'=>$from,'to'=>$p['lg'],'txt'=>$txt]);
return $voc;}
...
foreach($rb as $k=>$v)
	if(!isset($r[$v][$p['lg']])){$txt=''; $voc='';
		...
		//if($txt)$voc=yandex::com(['from'=>$from,'to'=>$p['lg'],'txt'=>$txt]);
...
if(!$ex)sqlup(self::$db,'ref',$ref,$p['id']);
else{
	$bid=sql('id',self::$db,'v',['ref'=>$old]);
	sqlup(self::$db2,'bid',$ex,['bid'=>$bid]);//trigger
...
$nid=sqlsavif(self::$db,['ref'=>$p['ref'],'lg'=>$p['lg']]);
return self::com($p);}

static function addfrom($p){
$p['voc']=yandex::com(['from'=>$p['from'],'to'=>$p['lg'],'txt'=>$p['ref']]);
$p['id']=sqlsav(self::$db,[$p['ref'],$p['lg']]);
return self::edit($p);}
...
if(!$p['id'])$p['id']=sqlsav(self::$db,[$ref,ses('lng')]);
...
sqldel(self::$db2,$p['bid']);
return self::usetags($p);}

static function addentry($p){
[$ref,$id,$a,$lg]=vals($p,['adtg','id','a','lg']);
if($ref)$nid=sqlsavif(self::$db,['ref'=>$ref,'lg'=>$lg]);
if($ref)$nid=sqlsavif(self::$db2,['bid'=>$nid,'aid'=>$id,'app'=>$a]);
return self::usetags($p);}

#call
static function taglist($a,$pub){
//$w=$pub?'and pub='.$pub:''; '.$w.'
return sqlin('tags.id as id,ref,count(bid) as nb',self::$db2,self::$db,'bid','','where lg="'.ses('lng').'" and app="'.$a.'" group by tags.id order by nb desc');}

static function build($p){
[$a,$id,$lg]=vals($p,['a','id','lg']);
//return sqlin('tags_r.id as id,tags.id as bid,ref',self::$db2,self::$db,'bid','rr',['aid'=>$id,'app'=>$a]);
return sqlrin('tags_r.id as id,tags.id as bid,ref',[[self::$db,'id',self::$db2,'bid']],'rr',['aid'=>$id,'app'=>$a]);}
...
if($r)foreach($r as $k=>$v){
	$t=$a::tit(['id'=>$v,'a'=>$a]);
	$c=$dsp==1?'licon':'cicon';
	$ret.=bj('popup|'.$a.',call|id='.$v,pic($a).$t,$c);}
return $ret;}
...
if($r)foreach($r as $k=>$v){[$a,$id]=$v; $t='';
	...
	$ret.=bj('popup|'.$a.',call|id='.$id,pic($a).$t,'cicon');}
return $ret;}
