...
sqlcreate(self::$db3,['bid'=>'int','cid'=>'int','uid'=>'int','vote'=>'int'],1);
sqlcreate(self::$db2,['bid'=>'int','uid'=>'int','competence'=>'bvar','score'=>'int'],1);
...
if($p['vote']=='ok'){$p['op2']=1;
	sqlsav(self::$db3,[$p['id'],$p['cid'],ses('uid'),1]);}
elseif($p['vote']=='ko'){
	sqldel(self::$db3,['bid'=>$p['id'],'cid'=>$p['cid'],'uid'=>ses('uid')]);}
$score=sql('count(id)',self::$db3,'v',['bid'=>$p['id'],'cid'=>$p['cid']]);
sqlup(self::$db2,'score',$score,$p['cid']);
return self::call($p);}

static function participation($p){
if($p['subscribe']=='ok'){$p['op1']=1;
	sqlsav(self::$db2,[$p['id'],ses('uid'),'',1]);}
elseif($p['subscribe']=='ko'){
	sqldel(self::$db2,['bid'=>$p['id'],'uid'=>ses('uid')]);
	sqldel(self::$db3,['bid'=>$p['id'],'uid'=>ses('uid')]);}
return self::call($p);}
...
list($ra,$rb)=self::build($p); $n=count($rb); $free=$ra['nb1']-$n;
