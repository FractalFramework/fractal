...
//sqlcreate(self::$db2,['bid'=>'int','uid'=>'int','val'=>'var'],1);//id,...,day
...
$gid=$g::idsuj($r['tit']); $f='usr/'.ses('user').'/algo/'.$id; $rb=db::read($f);
if(!$gid){$txt=tabler($rb);
	$ret=$g::save(['tit'=>$r['tit'],'txt'=>$txt,'type'=>1,'dk'=>0,'dv'=>0,'ad'=>1,'dc'=>1,'pr'=>0,'lb'=>0]);}
else{$fb='usr/'.ses('user').'/graphics/'.$gid; db::save($fb,$rb); $ret=$g::edit(['id'=>$gid]);}
return div($ret,'','tbl');}

static function mktable($p){$ret='';
$id=$p['id']; $a=val($p,'almin',0); $b=$p['almax']; $ia=$p['alinc'];
...
list($p,$c)=split_one(':',$da,1); //echo $da;
if($c=='x')self::$r[]=$p;}
...
foreach($ra as $k=>$v)$d=str_replace('['.$v.':x]',input('ex'.$v,$v,4,1,1,12,ajx($j)),$d);
