...
sqlcreate(self::$db2,['bid'=>'int','idn'=>'int','chapter'=>'var','txt'=>'long'],1);}
...
if($r)foreach($r as $k=>$v)sqlup(self::$db2,'idn',$k+1,$v);
...
case('imp'):$aid=$p['aid']??'';
	...
	if($rf)foreach($rf as $k=>$v){$i=$k+1;//save chapters
		...
		if($ex)sqlups(self::$db2,$rd,$ex); else $ex=sqlsav(self::$db2,$rd);}
...
if(auth(6))$ret.=bj($id.'dwl|book,dlop|op=imp,id='.$id,langp('import_art'),'btn');
return $ret;}
...
$ra=sqlin('name,tit,subt,bkg,img',self::$db,'login','uid','ra',$id);
...
$r=sqlin('name,tit,subt,bkg,img',self::$db,'login','uid','ra',$id);
...
$r=sqlin('name,tit,subt',self::$db,'login','uid','ra',$id);
$rb=sql('idn,chapter,txt',self::$db2,'ra',$idb);
$ret=div(bjk($cb.$id.'|book,call|mnu=1,id='.$id,pic('top').' '.$r['tit'],'','book/'.$id),'btit booknfo');
$nav=self::nav($p,$rb);
$ret.=div($nav,'booknfo');
...
static function play($p){$id=$p['id']??''; $idb=$p['idb']??''; $o=$p['o']??''; 
if(!$idb && $o){$idb=sql('id',self::$db2,'v',['bid'=>$id,'idn'=>$o]); if($idb)$p['idb']=$idb;}
if(!$idb)$ret=self::menu($p);
else $ret=self::reader($p);
return div($ret,'book',self::$cb.$p['id']);}

static function cover($id,$r=[]){$s='';
if(!isset($r['subt']))$r=sqlin('name,tit,subt,bkg',self::$db,'login','uid','ra',$id);
$ret=tag('h1','',$r['tit']);
