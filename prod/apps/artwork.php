...
list($typ,$com,$ic,$bt)=$v; $app=struntil($com,','); $aid=strend($com,'=');
if($typ=='img')$app=$typ; elseif($typ=='')$app='url'; $conn=$aid.'/'.$app;
$ret.=bj($cb.'subedt|'.$a.',subform|bid='.$p['bid'].',conn='.$conn,ico($ic).$bt,'');}
return div($ret,'list');}*/

static function subform($r){$cls=sqlcols(self::$db2,6); $a=self::$a; $cb=self::$cb;
...
$ra=sqlin('name,tit',self::$db,'login','uid','ra',$id);
$rb=sql('conn',self::$db2,'rr',['bid'=>$id]);
return [$ra,$rb];}

static function reader($p){$id=$p['id']??'';
$r=sqlin('name,tit',self::$db,'login','uid','ra',$id);
