...
sqlcreate('decide_args',['bid'=>'int','uid'=>'int','position'=>'int','txt'=>'bvar'],1);
sqlcreate('decide_chat',['cid'=>'int','uid'=>'int','txt'=>'bvar'],1);
sqlcreate('decide_valid',['cid'=>'int','uid'=>'int','val'=>'int'],1);
...
$r=sqlin('decide_args.id,decide_valid.id,val','decide_valid','decide_args','cid','kkv','where bid='.$id.' and (val=1 or val=2) order by cid');
...
if(!isset($id))$p['idc']=sqlsav('decide_valid',[$p['cid'],ses('uid'),$p['val']]);
else sqlup('decide_valid','val',$p['val'],$id);
return self::vote($p);}

static function vote($p){$cid=val($p,'cid'); $pos=val($p,'position'); $kid=randid('dcd');
...
if($p['idc'])sqldel('decide_chat',$p['idc']);
return self::panechat($p);}

static function savechat($p){
$p['idc']=sqlsav('decide_chat',[$p['cid'],ses('uid'),$p['txt'.$p['cid']]]);
return self::panechat($p);}
...
$r=sqlin($cols,$b,'login','uid','rr','where '.$b.'.cid='.$cid.' order by '.$b.'.id desc');
...
if($r)foreach($r as $v){//read
	...
	if($v['name']==ses('user'))$by.=span(bubble('decide,delchat|id='.$id.',cid='.$cid.',idc='.$v['id'].'',pic('del'),'btdel'),'right');
...
$r=sqlin($cols,'decide_args','login','uid','ra',$w);
...
if($r['name']==ses('user'))$bt.=bubble('decide,delarg|cid='.$cid.',id='.$id,pic('delete'),'btdel');
...
$r=sqlin($cols,self::$db,'login','uid','ra',$w);
