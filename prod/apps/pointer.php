...
//$r=sqljoin('tit,'.self::$db2.'.id',self::$db,self::$db2,'bid','kr',$id,1);
...
$ex=sqljoin('uid',self::$db3,self::$db2,'cid','v',['bid'=>$id,'uid'=>ses('uid')]);
$ra=sql('id,col',self::$db2,'kv',['bid'=>$id]); //pr($ra);
$rb=sqljoin('col,uid,'.self::$db3.'.id,cid',self::$db3,self::$db2,'cid','rr',['bid'=>$id]); //pr($rb);
