...
if(!isset($idv))$idv=sqlsav('poll_valid',array($id,ses('uid'),$p['val']));
else sqlup('poll_valid','val',$p['val'],$idv);
return self::play($p);}
...
$id=$p['id']; $ok=$p['fcp'.$id]??'';
if($ok){$com=sql('com',self::$db,'v',$id); sqlup(self::$db,'com',$com.'|'.$ok,$id);}
return self::play($p);}
