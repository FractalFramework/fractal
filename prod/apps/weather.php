...
//list($rb,$ra)=meteo::render('cities',$r); pr($rb);
...
if(!$rb or $x){$rb=meteo::$va($vr); $res=json_encode($rb); //pr($rb);//list($rb,$ra)??
	if($x){$ex=sql('id',self::$db2,'v',$prm); sqlups(self::$db2,['res'=>$res],$ex,0);}
	else{$ex=[$id,date('ymdH'),$r['insee'],$res];
		if(!$ex)sqlsav(self::$db2,[$id,date('ymdH'),$r['insee'],$res],0);}}
