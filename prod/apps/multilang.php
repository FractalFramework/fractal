...
//sqlcreate(self::$db2,['bid'=>'int','idb'=>'int','lang'=>'var'],1);
...
if(isset($idb)){
	list($t,$d)=sql('tit,txt',self::$db,'rw',$idb);
	$t2=yandex::com(['from'=>$lg,'to'=>$lang,'txt'=>$t]);
	if($t2)sqlup(self::$db,'tit',$t2,$id);
	$d2=yandex::com(['from'=>$lg,'to'=>$lang,'txt'=>$d]);
	if($d2)sqlup(self::$db,'txt',$d2,$id);}
return self::edit($p);}

static function newfrom($p){
$id=$p['id']??''; $lang=val($p,'lang'); $p['opn']=1;
list($t,$d,$lg,$md5)=sql('tit,txt,lang,md5',self::$db,'rw',$id);
$t2=yandex::com(['from'=>$lg,'to'=>$lang,'txt'=>$t]);
$d2=yandex::com(['from'=>$lg,'to'=>$lang,'txt'=>$d]);
$p['id']=sqlsav(self::$db,[ses('uid'),$t2,$d2,$lang,$md5,0,0]);
return self::edit($p);}

static function translations($p){$ret='';
$id=$p['id']??''; $lang=val($p,'lang'); $md5=val($p,'md5');
...
//$p['lg']=ses('mlg',val($p,'lang'));//,ses('lng')
