...
sqlcreate(self::$db2,['bid'=>'int','day'=>'int','prm'=>'var','res'=>'text'],1);
...
add_head('csslink','/css/weather.css');
...
if(!$rb or $x){list($rb,$ra)=meteo::$va($vr); $rs=json_encode($rb); //pr($rb);//eco($rs);
	$time=$rb['time'][0]??($rb['time']??''); if(!$time)$time=$r['up'];
	if($x){$id=sql('id',self::$db2,'v',['bid'=>$id]); sqlups(self::$db2,['res'=>$rs],$id,1);} else
	sqlsav(self::$db2,[$id,strtotime($time),$prm,$rs],0);
