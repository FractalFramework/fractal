...
sqlcreate(self::$db2,$r,1);
parent::install(array_combine(self::$cols,self::$typs));}

static function admin($p){$p['o']='1'; return parent::admin($p);}
static function injectJs(){return 'function sldbar(v,nid){
	var id=nid.substr(6);
...
add_head('csscode','
.slide{background:black; color:white; font-size:x-large; font-family:Ubuntu; align:center; padding:100px; margin:10px 0 0 0; white-space:pre-wrap; height:440px; overflow-y:auto; overflow-x:hidden;}');
...
$p['id']=sqlsav(self::$db,['uid'=>ses('uid'),'tit'=>$tit,'pub'=>0,'edt'=>0]);
sqlsav(self::$db2,[$p['id'],'1','0',lang('first slide'),'',lang('first slide'),'0']);
return self::edit($p);}
...
#subedit
static function subbt($p){$id=$p['id']; $idb=val($p,'idb'); $ret='';
...
foreach($rb as $k=>$v)sqlups(self::$db2,$v,$k);//save
foreach($rb as $k=>$v)//rapport
	foreach($v as $ka=>$va)if($va!=$r[$k][$ka] && $r[$k][$ka])$rc[$k][$ka]=$ka.':'.$r[$k][$ka].'->'.$va;
return $rc;}

static function collisions($p){$db2=self::$db2; $ret='';
$id=$p['id']??''; $idb=val($p,'idb'); $idn=val($p,'idn'); $idp=val($p,'idp'); $rel=val($p,'rel');
list($_idn,$_idp,$_rel)=sql('idn,idp,rel',$db2,'rw',$idb);
...
$ra=valk($p,['t2','bkg','txt']);
sqlups($db2,valk($p,['t2','bkg','txt']),$idb);
$ra=valk($p,['idn','idp','rel']);
//sqlups($db2,$ra,$idb);
...
$p['t']='t2'; $p['bt']=self::subbt($p);
$id=$p['id']??''; $idb=val($p,'idb'); $ret='';
$cb=self::$cb; $a=self::$a; $db2=self::$db2;
if(val($p,'op')=='sav'){$p['op']='';
	$r=valk($p,sqlcols($db2,6));
	//sqlups($db2,$r,$idb);
	$ret=self::collisions($p);}//alternative save
elseif(val($p,'op')=='add')$p['idn']=val($p,'idn')+1;
...
$ok=val($p,'ok'); $delall=val($p,'delall'); $rid=$p['rid']??'';
$id=$p['id']??''; $idn=val($p,'idn'); if($idn==1)$idn=0;//forbid del first slide
...
elseif($id && $delall){sqldel(self::$db,$id); sqldel(self::$db2,$id,'bid'); 
	return self::stream($p);}
elseif($id && $idn){//reorder slides and parents after deleting
	qr('delete from '.self::$db2.' where bid="'.$id.'" and idn="'.$idn.'"');
	$r=sql('id,idn',self::$db2,'kv','where bid="'.$id.'" and idn>"'.$idn.'" order by idn');
	if($r)foreach($r as $k=>$v){$nidn=$v-1; sqlup(self::$db2,'idn',$nidn,$k);}
	$r=sql('id,idp',self::$db2,'kv','where bid="'.$id.'" and idp>="'.$idn.'" order by idn');
	if($r)foreach($r as $k=>$v)if($v){$nidp=$v-1; sqlup(self::$db2,'idp',$nidp,$k);}
...
$id=$p['id']??''; $idb=val($p,'idb'); $mdf=val($p,'mdf'); //$idn1=val($p,'idn1');
if($mdf)sqlups(self::$db2,valk($p,['idn','idp','t2','bkg','txt','rel']),$idb);
else $id=sqlsav(self::$db2,valk($p,['bid','idn','idp','t2','bkg','txt','rel']));
//if($p['idn']!=$idn1 && $idn1)self::collisions($p);
return self::play($p);}

static function addslide($p){
$rid=$p['rid']??''; $id=val($p,'id',1);
$idn=self::lasidn($id); $idp=val($p,'idn'); $idp='';
...
$rid=$p['rid']??''; $id=val($p,'id',1);//id=bid
$idn=val($p,'idn',1); $idn=val($p,'idn',1); $idb=val($p,'idb');//idb=id slide
...
foreach($r as $k=>$v){$idb=$rt[$k][0]; $t=$k.'. '.$rt[$k][1];
	if($k && $edt)$bt=bj($cb.$id.'sub|slide,subedit|id='.$id.',idb='.$idb,$t,'');
	elseif($k)$bt=bj('sld'.$id.'|slide,play|id='.$id.',idb='.$idb,$t,'');
	if(is_array($v))$bt.=self::topo_r($id,$v,$rt,$edt);
	$ret.=li($bt,'');}
if($ret)return ul($ret);}

static function topo($p){$id=$p['id']??''; $edt=val($p,'edt');
...
$txt=val($r,'txt'); $txt=conn::com($txt,1);
...
list($tit,$id,$rid,$own)=vals($p,['tit','id','rid','own']);
$cb=self::$cb.$id; $idb=$ra['id']; $idn=$ra['idn'];
$prm='id='.$id.',rid='.$rid; $j=$cb.'|slide,play|'.$prm;
$bt=bj($j.',idn=1',pic('slide').' '.$tit,'btn');
if($own){
	$btb.=bj('popup|slide,mdfslide|'.$prm.',idb='.$idb,langpi('modif'),'btn');//subedit
	$btb.=bj('popup|slide,addslide|'.$prm.',idn='.$idn,langpi('add'),'btsav');
	$btb.=bj('popup|slide,sldel|'.$prm.',idb='.$idb,langpi('del'),'btdel');
	$btb.=bj($cb.'|slide,edit|'.$prm.',idb='.$idb,langpi('edit'),'btn');}
...
$id=$p['id']??''; $idb=val($p,'idb'); $idn=val($p,'idn',1);
