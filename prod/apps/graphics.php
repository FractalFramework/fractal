...
sqlcreate(self::$db2,['uid'=>'int','tit'=>'var','txt'=>'text','cfg'=>'json','pub'=>'int'],1);
...
//pr($rb);
trunc('graphics2'); sqlsav2('graphics2',$rb,5);
rntable('graphics','graphics0');
rntable('graphics2','graphics');
}
...
$txt=self::trans(val($p,'txt'));//$r=explorer::repair($r);
$rb=valk($p,self::$json); $cfg=json_encode($rb);
$r=[ses('uid'),$p['tit'],$txt,$cfg,$p['pub']];
//$r=validcols($r,$db);
$p['id']=sqlsav($db,$r);
...
$rp['r']=db::read($f); $rp['t']='graphics'.$p['id']; $rp['im']=$im; //pr($rp);
