...
$u='disk/usr/'.ses('user').'/json/'.$f.'.json';
//$d=self::divide($u,10);
$sz=filesize($u); //140521653
$l=10000000; //$l=2;
$rk=['nb'=>'int','txt'=>'longtext']; $db='aaa_txt';
//sqlcreate($db,$rk,1);
$n=16;
if(!sql('id',$db,'v',$n)){
list($min,$max)=self::parts($sz,$n,$l); //echo $min.'-'.$max.' ';
...
//sqlsavif(self::$db,['tit'=>$fa,'txt'=>$db]);
//$r=json::read($u);
$cols=[]; //pr($r);
//if($cols)sqlcreate($db,$cols,1); sqlsav2($db,$r,0);
...
sqlcreate($db,$rk,1); sqlsav2($db,$rb,5);
//pr($rb);
$ret=count($rb);
