...
if($p)switch($p){
	...
	case('dec-b36'):$d=b36($d); break;
	case('b36-dec'):$d=b36($d,1); break;
	...
	case('powers'):$d=html_entity_decode(maths::powers($d)); break;
	case('measures'):$d=maths::measures($d); break;
	case('magnitude'):$d=maths::magnitude($d); break;
	case('numerology'):$d=maths::numerology($d); break;
	case('xyz'):list($ad,$dc,$ds)=expl(',',$d,3);
		$r=maths::xyz((float)$ad,(float)$dc,(float)$ds); $d=implode(n(),$r); break;
	default:if(method_exists('maths',$p))$d=maths::$p($d); elseif(function_exists($p))$d=$p($d); break;}
return $d;}
...
'astro'=>['al2km','km2al','al2pc','pc2al','km2pc','pc2km','pc2mas','mas2pc','al2mas','mas2al','deg2mas','mas2deg','mas2rad','ra2deg','deg2ra','dec2deg','deg2dec','nm2thz','cm2hz'],
'3d'=>['xyz','test']];}
