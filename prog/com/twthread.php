...
#play
static function playsub($t){$ret='';
$l=strlen($t); $n=false; $s=1; if($l>180)$n=180; else $n=$l;
$tb=substr($t,0,$n); $nb=strpos($tb,'--'); if($nb!==false){$n=$nb; $s=2;}
$tb=substr($tb,0,$n); $nb=strrpos($tb,'.'); if($nb!==false)$n=$nb+1; //echo ':'.$n.'-';
$tb=substr($tb,0,$n);
if($tb)$ret.=div(trim($tb),'pane');
if($l>$n)$ret.=self::playsub(substr($t,$n+$s));
if($ret)return $ret;}

static function play($p){
$ret=self::playsub(val($p,'txt'));
return div($ret,'txt','twt','white-space:pre-line;');}
...
$bt=self::menu($p);
$ret=self::call($p);
return div(div($bt,'col1').div($ret,'col2 pane scroll','','magrin:0 10px;'),'grid');}
}
?>