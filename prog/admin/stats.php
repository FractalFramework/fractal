...
sqlcreate(self::$db2,['iq'=>'int','app'=>'svar','prm'=>'svar'],1);
sqlcreate(self::$db3,['day'=>'int','du'=>'int','vu'=>'int'],1);}
...
if($r)sqlsav2('stats_c',$r);
return tabler($r);}

#build
static function build($p){$id=$p['id']??''; $n=val($p,'n',0); $db=self::$db; $db2=self::$db2; $na=500;
$nb=($n*$na); $l=$nb.','.($nb+$na);
$r=sqlrin('ip,uid,app,prm,'.$db.'.ip,numsec',[[$db2,'iq',$db,'id']],'','order by '.$db2.'.up desc limit '.$l);
return $r;}
