...
function updateurl(u,j){var n=u.indexOf('/'); //return;
	var a=n?u.substring(0,n):u; var p=n?u.substring(n+1):''; if(j=='undefined')j='';
	var r={urlPath:u,'app':a,'p':p,'j':j}; if(u!='/')u='/'+u;
...
function loadp(e){
	...
	var j='page|'+a+'|'+p;
	var r={urlPath:u,'app':a,'p':p,'j':j}; document.title=a;
	window.history.replaceState(r,a,u);}

window.onpopstate=function(e){var j=e.state.j; ajx(j?j:'page|home');}
//window.onload=function(){location.assign(document.url);}
...
function maxlength(id,limit){var tx=getbyid(id).innerHTML;
...
function insert(text,id,el){var ob=getbyid(id,el);
...
function wygedt(e,id,aid){var d=getrange(id);
	var prm='id:'+aid+',start:'+d.start+',end:'+d.end+',txt:'+jurl(d.txt);
	var url='/call.php?appName=build&appMethod=wygedt&popup==&params='+prm;
	if(d.txt)var ajax=new AJAX(url,'popup','','');}

function wygedt2(id){var d=getrange(id);
	if(d.txt)ajx('bubble|stabilo,add_note|id='+aid+',start='+d.start+',end='+d.end+',txt='+jurl(d.txt));}

//diveditable
function insertChar(t,id){var ob=getbyid(id);
...
function completion(id){var ob=getbyid(id);
...
function strcount2(rid){var limit=768; var on=500; 
	...
	//if(tx.length>limit)tb.className='btno'; else tb.className='btsav';
	if(tx.length>limit)tb.style.display='none'; else tb.style.display='';}

function autoResizeHeight(rid){var that=rid?getbyid(rid):this;
	that.style.height=0; that.style.height=(that.scrollHeight+7)+"px";}
...
function dom_r(ob,o=0){if(o==10)return;
...
//editor
function editor(id,ev){var ob=getbyid(id); idd=id; //var od=getbyid('addpst'+id);
...
function dedicated_div(tar,n){var ob=getbyid(tar); if(ob==null)return;
...
function checkj(e,o){
	...
	if(char==13){var d=o.dataset.j; if(d)ajx(d,o); return false;}
	else return true;}
...
function dropok(ev,rid){ev.stopPropagation(); ev.preventDefault(); var fd=new FormData(); uploaded=0;
	var dt=ev.dataTransfer; var rf=dt.files; var n=rf.length;
	for(var i=0;i<n;i++)upim(rf[i],fd,rid);}

function upim(file,fd,rid){var time=Date.now(); //.name,.size
	var xtr=file.name.split('.'); var xt=xtr[xtr.length-1];
	if(file.type.match('image.*'))var ty='img'; else return;
	var filename=''+time+'.'+xt;
	fd.append('upl'+rid,file,filename);
	if(filename)upload_progress(rid);
	var url='/call.php?appName=dragdl&appMethod=upload&params=rid:'+rid+',ty:'+ty;
...
function applyclr(e,o){var d=e.value; var d1=''; var d2='';
...
function affectclr(c,id,o){var e=getbyid(id); var d=e.value;
