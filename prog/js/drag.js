...
function drag_start(ev){//pr("dragStart");
	...
	//var img=new Image(); img.src='http://telex.ovh/img/full/webf4eafea934.jpg'; evd.setDragImage(img,10,10);
...
function dropok(ev){ev.stopPropagation(); ev.preventDefault();
	var dt=ev.dataTransfer; var rf=dt.files; var n=rf.length; output("File Count: "+n+"\n",ev.target);
	for(var i=0;i<rf.length;i++){pr(rf[i]);
