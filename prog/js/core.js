...
//states
function updateurl(u,j){var n=u.indexOf('/'); //pr(u); //return;
...
function startstate(index){//pr(index);
	...
	var j='page|'+index+',content|app:'+a+',p:'+p; //pr(j);
...
function autoResizeHeight(rid){var ob=getbyid(rid);
...
function gps_ko(error){switch(error.code){
	...
	case error.TIMEOUT: pr('gps: ne répond pas'); break;}}
...
function savtim(id){if(sok){xa=setTimeout("savtim("+id+")",10000);
...
function savtxt(div,id,opn=0){var ob=getbyid(div+id);
