//Fractal GNU/GPL
var wait=0,popnb=0,curid=0,curh=0;cpop=0,cpop_difx=0,na=0,xt=0,cpop_dify=0,popz=1,fixpop=1,amt=0,cutat=2000;
...
if(inp!=undefined && inp!=null){
	for(var i=0;i<inp.length;i++){
		if(inp[i]){
			...
			if(ob==null){var ob=document.getElementsByName(inp[i]); var rc=[];//checklists
...
if(cbOption=='reload')setTimeout('window.location=document.URL',100);//,,reload
	...
	var did=getbyid(inp[i],el); if(did.type=='textarea')did.value=''; else did.innerHTML='';}
...
if(cbJs){
	...
	else if(cbJs){if(cbJs=='1')cbJs='injectJs';
		var url='/call.php?appName='+appName+'&appMethod='+cbJs;
		var ajax=new AJAX(url,'injectJs','','2','','');}}}

function ajb(p,el){
	var com=p[0]; var app=p[1]; var prm=p[2]; var inp=p[3]; var cbk=p[0].split(','); var z=cbk[0];
	if(!cbk[1] && z!='popup' && z!='pagup' && z!='imgup' && z!='bubble' && z!='menu' && z!='drop' && z!='input' && z!='before' && z!='after' && z!='begin' && z!='atend' && z!='reload' && z!='ses' && z!='socket' && z!='injectJs' && z!='json'){cbk[0]='div'; cbk[1]=z;}
	//var r=['popup','pagup','imgup','bubble','menu','drop','input','before','after','begin','atend','reload','ses','socket','injectJs','json']; if(!cbk[1] && r.indexOf(z)!=-1){cbk[0]='div'; cbk[1]=z;} pr(r.indexOf(z));
...
function ajbt(el){var p2='';
	...
	var tab=el.dataset.tab; if(tab)togtab(el);}
...
function toggle(ob){var res=0; var did=ob.dataset.toggle; var bid=ob.dataset.bid; var ko=ob.dataset.ko; if(ko==0)ko='';
	if(ob.rel==1 && !ko){closediv(did,ob); ob.rel=''; active(ob,0); res=1;//restored
		var jb=ob.dataset.jb; if(jb)ajx(jb,ob); if(bid){var oba=getbyid(bid); active(oba,1); oba.rel=1;}}
...
//Amt
function multithread(content,ix){localStorage['m2']=content;
na=Math.ceil(content.length/cutat); if(!na)na=1; rs=0;
for(i=0;i<na;i++){var res=encodeURIComponent(content.substr((i*cutat),cutat));
...
//bubjs
function bubjs(res,id,o){
var mp=mouse(event); var pp=getbyid('bubjs');
...
if(o==1){pp.style.display='inline-block'; pp.innerHTML=res; pp.style.position='absolute';
	pp.style.top=(mp.y-30)+'px'; pp.style.left=(mp.x-8)+'px';}//pr(res);
...
var arr=["\n","\t",'\'','|',"'",'"','*','#','+','=','&','?','.',':',',','§','%u'];//,' ','<','>','/'
var arb=['(-n)','(-t)','(-asl)','(-bar)','(-q)','(-dq)','(-star)','(-dz)','(-add)','(-eq)','(-and)','(-qm)','(-dot)','(-ddot)','(-coma)','(-par)','(-pu)'];//,'(-sp)','(-b1)','(-b2)','(-sl)'
...
var js=document.createElement('script'); js.id='addjs'; var ob=getbyid('addjs'); if(ob!=null)head.removeChild(ob);
js.type='text/javascript'; js.innerHTML=d; head.appendChild(js);}
