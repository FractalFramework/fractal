...
foreach($r as $k=>$v)if(strpos($v,$a)){$v=str_replace($a,$b,$v); sqlup('tlex','txt',$v,$k); echo $k.' ';}
return $ret;}
...
if($r)foreach($r as $k=>$v)if($v)sqlsavif('tlex_tag',['tlxid'=>$id,'tag'=>$v]);}

static function savemnt($id,$r){
if($r)foreach($r as $k=>$v)if($v)sqlsavif('tlex_mnt',['tlxid'=>$id,'tousr'=>$v]);}
...
foreach($r as $k=>$v){
	list($id,$uid,$dir,$type,$com,$picto,$bt,$auth)=$v;
