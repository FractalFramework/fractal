...
if(ses('usr'))$r[]=[$root,'j',',,reload|login,disconnect','','logout'];
else $r[]=[$root,'pop','login,com','login','login'];
return $r;}
...
static function dev($r,$a){$dev=ses('dev');
if($dev=='prog'){
	...
	if(auth(6)){$r[]=[$dev.'','pop','admin_lang','','lang'];
		...
		$r[]=[$dev.'/sys','pop','editapp','edit','edit'];
...
$a=$p['a']; $dev=ses('dev');
$r=self::about($a);
if($dev=='prog')$r[]=[$dev.'','j','ses,,reload||k=dev,v=prod','','prod'];
return $r;}
