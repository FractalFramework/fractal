...
if(!$id)sqlsav('freevote_valid',[$bid,ses('uid'),$p['val']]);
elseif($id){$v=$p['val']==$p['current']?0:$p['val']; sqlup('freevote_valid','val',$v,$id);}
return self::vote($p);}

static function vote($p){
$id=$p['id']??''; $bid=val($p,'bid'); $kid=randid('freevote'); $cl=val($p,'cl'); $nb=val($p,'nbvotes');
$vote=sql('val','freevote_valid','v','where cid="'.$bid.'" and uid="'.ses('uid').'"');
$tot=val($p,'tot'); $median=val($p,'median'); $medp=val($p,'medp'); $bmed=val($p,'bmed'); $n='';
...
$id=$p['id']??''; $bid=val($p,'bid'); $cl=val($p,'cl'); //$tot=val($p,'tot');
...
$r=sqlin($cols,'freevote_args','login','uid','ra',$w.' order by nb desc');
...
$nv=sqljoin('count(freevote_valid.id)','freevote_valid','freevote_args','cid','v','where bid="'.$id.'"');
$p['nbvotes']=$n;
