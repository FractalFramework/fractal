...
sqlcreate(self::$db2,['uid'=>'int','bid'=>'int','start'=>'int','end'=>'int','pad'=>'var','txt'=>'bvar'],1);
...
//$r=sqlin('name,pad,txt',self::$db2,'login','uid','ra','where '.self::$db2.'.id='.$id);
$r=sqlrin('name,pad,txt',[[self::$db2,'uid','login','id']],'ra',[self::$db2.'.id'=>$id]);
...
//$usr=sqljoin('name',self::$db2,'login','uid','v','where '.self::$db2.'.id='.$id);
$usr=sqlrin('name',[[self::$db2,'uid','login','id']],'v',[self::$db2.'.id'=>$id]);
