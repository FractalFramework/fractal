...
$r=sqlin('distinct(uid),name',self::$db2,'login','uid','kv','where bid="'.$id.'"');
...
if(!$idnote)$p['idnote']=sqlsav(self::$db2,[$id,ses('uid'),$p['choice'],$p['val']]);
...
foreach($rb as $k=>$v){$me=$rm[$v]; $ca=$rt[$v][$me];
	list($cq,$cp)=self::scores($rt[$v],$me,$u); //pr($rt[$v]);
...
list($k1,$v1)=self::maxk($rd1); list($k2,$v2)=self::maxk($rd2);
...
static function algo($r,$m=0.5,$a=2){
$rm=[]; $rt=[]; $rx=[]; //pr($r);
...
if($rc[$ka]??'')$score=array_sum(array_slice($rc[$ka],$mnt/2)); $sc=round($score*100,2);
...
[$answers,$end]=sql('com,day',self::$db,'rw',$id); $a=3;//mode of exaequo
if(ses('uid'))$note=sql('choice,val',self::$db2,'kv','where bid="'.$id.'" and uid="'.ses('uid').'"'); else $note=[];
$rs=sql('distinct(uid)',self::$db2,'k','where bid="'.$id.'" order by val');
