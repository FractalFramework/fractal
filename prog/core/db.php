...
static function secu($f,$x){$rf=explode('/',$f);
	if($rf[1]??''==ses('user') or $x or auth(6))return 1; else echo '!';}
...
$prm='f='.$f.',k='.$k.'|v'.$k;
$ret=input('v'.$k,$v).bj($k.'|db,sav|'.$prp,langpi('modif'),'btsav');
...
if($r){foreach($r as $k=>$v){$i++; if($k==0)$k=$i; $rb[$k]=is_array($v)?$v:[$v];}
return $rb;}}
