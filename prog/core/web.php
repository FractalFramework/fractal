...
$ti=dom_extract($dom,'::h1');
if(!$ti)$ti=dom_extract($dom,'title:property:meta');
if(!$ti)$ti=dom_extract($dom,'og(ddot)title:property:meta');
if(!$ti)$ti=dom_extract($dom,'name:itemprop:meta');
if(!$ti)$ti=dom_extract($dom,'::title');
if(!$ti)$ti=dom_extract($dom,'::h1');
$tx=dom_extract($dom,'description:name:meta'); //$x=''; if($tx)$x.=1;
if(!$tx)$tx=dom_extract($dom,'og(ddot)description:property:meta');
if(!$tx)$tx=dom_extract($dom,'description:itemprop:meta');
//if(!$tx)$tx=embed_detect($d,'"description":"','",');//bad res
$im=dom_extract($dom,'image:name:meta');
if(!$im)$im=dom_extract($dom,'og(ddot)image:property:meta');
if(!$im)$im=dom_extract($dom,'og(ddot)image:itemprop:meta');
if(!$im)$im=dom_extract($dom,'thumbnailUrl:itemprop:link:href');
if(strpos($f,'youtube.com')!==false)$im='https://img.youtube.com/vi/'.strend($f,'=').'/hqdefault.jpg';
return [$ti,$tx,$im];}

static function build($d,$x=''){$im=''; $txt='';
...
if(!$r or $x){$ra=$r;// or !$r[0]
	...
	if($r[1])$txt=(str_replace(['“','”'],'',$r[1]));//html_entity_decode
	if($r[2])$im=saveimg($r[2],'web','590','400'); else $im='';
	if($ra)sqlups('tlex_web',['tit'=>trim($r[0]),'txt'=>trim($txt),'img'=>$im],$ra[3]);
	elseif($r[0])sqlsav('tlex_web',[$d,trim($r[0]),trim($txt),$im]);}
return $r?$r:['','',''];}
