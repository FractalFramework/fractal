...
#call //p:structure
static function call($p){$ret=[]; $uid=ses('uid');
$dir=val($p,'dir'); $app=val($p,'app'); $mth=val($p,'mth');
$cuid=val($p,'cuid'); $bid=val($p,'bid'); $rid=$p['rid']??'';
$dsp=ses('dskdsp',val($p,'display')); if(!$dsp)$dsp=ses('dskdsp',2);
$spd=ses('dskspd',val($p,'spread')); if(!$spd or !$uid)$spd=ses('dskspd',3);
$cmb=ses('dskcmb',val($p,'combine',1));
...
if(isset($r))foreach($r as $k=>$v){
	...
	if($auth>=$private){
		...
		if($v[0]==$dir or $cmb==2){
			if($v[1]=='img')$ico='';
			$bt=$ico.$v4;
			if(!$ok)$bt.=span('('.usrid($v[6]).')','small');
			$j='kedt|desktop,modifbt|dir=/'.$current_level.',id='.$k.',rid='.$rid.'|mdfbt';
			if($ok)$bt.=bj($j,picto('edit',12));//modifbt
			if($v[1]=='' && class_exists($v[2]))$ret[]=span(popup($v[2].'|headers=1',$bt),$css);
			elseif($v[1]=='j')$ret[]=span(bj($v[2],$bt),$css);
			elseif($v[1]=='ju')$ret[]=span(bjk($v[2],$bt,'','/'),$css);
			elseif($v[1]=='jk')$ret[]=span(bjk($v[2],$bt,'',$v[4]),$css);
			elseif($v[1]=='pop')$ret[]=span(popup($v[2].',headers=1',$bt),$css);
			elseif($v[1]=='pag')$ret[]=span(pagup($v[2].',headers=1',$bt),$css);
			//elseif($v[1]=='img')$ret[]=imgup('img/full/'.$v[2],$ico,$css);
			elseif($v[1]=='img')$ret[]=div(playimg($v[2],'micro').$bt,$css);
			elseif($v[1]=='audio')$ret[]=span(popup('core,audio|u='.nohttp($v[2]),$bt),$css);
			elseif($v[1]=='video')$ret[]=span(pogup('core,video|u='.nohttp($v[2]),$bt),$css);
			elseif($v[1]=='in')$ret[]=div(apj($v[2]));
		...
		elseif(substr($v[0],0,strlen($dir))==$dir && $depht>$current_depht){//dir
			...
			$bt=$ico.span($v4);//pic('edit',12)
			if($ok)$bt.=bj('kedt|desktop,modifdir|rid='.$rid.',dir='.$dir.'/'.$next_level,picto('edit',12));
			$ret[$next_level]=span(bj($rid.',,2|desk,call|dir='.$dir.'/'.$next_level.','.$prm,$bt),$css);}}}
//nav
...
if($app=='desktop'){
	...
	if(ses('uid'))$edit.=bj('cbck|desktop,renove|dir='.$dir,langpi('update'),'');
	$edit.=lk('/'.$app.'/dir:'.substr($dir,1),ico('link'),'');}
$edit.=span('','','kedt');
$edit=span($edit,'nbp');
if($ret)return div($back.$edit).implode('',$ret);
elseif($dir){
