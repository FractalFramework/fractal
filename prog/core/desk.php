...
$dir=val($p,'dir'); $app=$p['app']??''; $mth=val($p,'mth');
$cuid=val($p,'cuid'); $bid=$p['bid']??''; $rid=$p['rid']??'';
...
if(isset($r))foreach($r as $k=>$v){
	...
	if($auth>=$private){
		...
		if($v[0]==$dir or $cmb==2){
			...
			if($v[1]=='' && class_exists($v[2]))$rt[]=bj('cbck|'.$v[2].'|headers=1',$bt);
			...
			elseif(substr($v[0],0,strlen($dir))==$dir && $depht>$current_depht){//dir
				...
				$bt=div($ico.span($v4));//pic('edit',12)
...
$edit.=span('','','kedt');
$edit=span($edit,'nbp');
if($rt)return div($back.$edit).div(implode('',$rt),$css);
elseif($dir){
