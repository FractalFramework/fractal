...
static function proportion($r,$a,$b,$l,$o){$c=$b-$a; if(!$r)return;
if($l)foreach($r as $k=>$v)if($v>$l)$r[$k]=$l;
list($min,$max)=self::minmax($r); $diff=$max-$min; if(!$diff)$diff=1; $m=$c/$diff; $rb=[]; $ca=1/$diff;
foreach($r as $k=>$v){if(!is_numeric($v))$n=0.5; else $n=($v-$min)*$ca;//*$m
	if($o)$n=1-$n; $n=$n*$c+$a; $rb[$k]=$n;}
return $rb;}

static function sz($r){$rb=[]; //pr($r);
//$r0=array_column($r,'star');
$ra=array_keys_r($r,'radius'); $r1=self::proportion($ra,2,12,50,0); //pr($r1);
$rb=array_keys_r($r,'dist'); $r2=self::proportion($rb,2,12,100,1); //pr($r2);
if($r)foreach($r as $k=>$v){$rb[$k]=round((($r1[$k]??10)+$r2[$k]??0.1)/3,2);} //pr($rb);
return $rb;}
...
$n=94; for($i=0;$i<$n;$i++)$r[]=self::declinaison($a,$n,$i,4);//fourth
$n=88; for($i=0;$i<$n;$i++)$r[]=self::declinaison($a,$n,$i,1);//first
$n=90.5; for($i=0;$i<$n;$i++)$r[]=self::declinaison($a,$n,$i,2);//second
$n=92.5; for($i=0;$i<$n;$i++)$r[]=self::declinaison($a,$n,$i,3);//third
...
$n=90; for($i=0;$i<$n;$i++)$r[]=self::declinaison($a,$n,$i,2);
...
elseif($p1){$sq=self::sq($p1); $rb1=array_keys_r($rb,1); $rb2=array_keys_r($rb,0); $r2a=[]; $r2b=[]; //pr($rb1);
