...
static $db='';
static $cb='msc';
static $algs=[1=>'linear','power','music','fibo','exp','atomic'];
static $hz440=261.63;
static $hz432=256.87;
static $algo=3;
...
$al=self::$algo; $algo=self::$algs[$al];//music
if($algo=='music')return $d*(2**bcdiv($i,$s));
if($algo=='linear')return $d+bcdiv($d,$s/($s-$i));
if($algo=='power')return $d*bcdiv($d,$s/($s-$i));
if($algo=='fibo')return val($r,$i-2,1)+val($r,$i-1,1);
if($algo=='exp')return exp($i);
if($algo=='atomic')return $d*$i*4;}
...
static function graph_notes($mi,$h,$do){
...
$ret.=self::graph_waves_spi($mi,$h1,$do,$d,1);
//sound
$h1=250; $ret.=self::graph_waves($mi,$h1,$da,'',0);
//notes
$h1=320; $ret.=self::graph_notes($mi,$h1,$da);
//echo $ret;
return svg::call(['code'=>$ret,'w'=>$w,'h'=>$h]);}

static function octaves($m){//$d**$i
if($m=='432')$d=2.04375; elseif($m=='440')$d=2.04375; else $d=2;//Hz
for($i=$d;$i<=64;$i++){$a=2**$i; $r[]=[$i,$a,maths::powers($a,'Hz')];}
return $r;}

static function wavelengths($m){//not good calculation
if($m=='432')$d=2148.96; elseif($m=='440')$d=2109.89; else $d=2100;//cm
for($i=$d;$i<=64;$i++){$a=2**$i; $b=maths::nm2thz($a);//cm2hz
	$r[]=[$i,$b,maths::powers($a,'cm')];}//magnitude
return $r;}
...
$rb['_']=['','Hz'];
$ro=self::octaves($m);
//$ro=self::wavelengths($m);
if($m=='432')$d=self::$hz432;//LA at 432Hz
elseif($m=='440')$d=self::$hz440;//LA at 440Hz
else $d=$ro[$v][1];//6:256 
$ret=self::graph($ro,$d);
$r=self::build($d);
...
$bt.=bj($j.'mode=432|inp2','432Hz','btn');
$bt.=bj($j.'mode=440|inp2','440Hz','btn');
//$bt.=radio('algo',self::$algs,'3');
