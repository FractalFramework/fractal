...
//$r[]=['','lk','/spitable2','','spitable2'];
return $r;}

static function injectJs($j,$n=1,$o){//self::patch();
$atoms=db_read('db/public/atomic','','',1);
$ra=$atoms?json_encode(utf_r($atoms)):''; $j='spt|spitable,call';
return 'var n='.$n.';
...
//add_head('jscode',self::injectJs('spt|spitable,call',$p,$o));
...
$s='12px'; $j='[spt|spitable;call|p1='.$max; $s='font-size:12px';
...
$t1='[spt|spitable;call|p1='.$i.',|mode§['.($x+$mg).','.($y+$mg2).','.$s.'§'.$i.':text]:bj]';
$t2='[popup|spitable;infos|p1='.$i.'§['.($x+$mg).','.($y+$h-$mg2).','.$s.'§'.$v.':text]:bj]';}
else{$w=$u*2; $h=$u;
$t1='[spt|spitable;call|p1='.$i.'|mode§['.($x+$mg).','.($y+$mg2).','.$s.'§'.$i.':text]:bj]';
$t2='[popup|spitable;infos|p1='.$i.'§['.($x+$w/2+6).','.($y+$mg2).','.$s.'§'.$v.':text]:bj]';}
...
$j=$rid.'|spitable,call||lblbar,mode';
...
add_head('jscode',self::injectJs('spt|spitable,call',$a,$o));
$ret=self::call($p);
