...
static function injectJs($j,$n=1,$o){return 'var n='.$n.';
...
//add_head('jscode',self::injectJs('spl|spitable2,call',$p,$o));
add_head('csscode',self::css());}

static function clr0(){return [''=>'ccc','Nonmetals'=>'92FF10','Nobles Gasses'=>'05FFFF','Alkali Metals'=>'FF9801','Alkali Earth Metals'=>'BF6700','Metalloids'=>'91C9D6','Halogens'=>'FFFF00','Metals'=>'ABABB0','Transactinides'=>'C9C97A','Lanthanides'=>'B3D7AB','Actinides'=>'75ADAB','undefined'=>'ffffff'];}
static function clr1(){return [''=>'ccc','Nonmetals'=>'e8592d','Nobles Gasses'=>'fbbf09','Alkali Metals'=>'c1cf00','Alkali Earth Metals'=>'95a112','Metalloids'=>'6b7612','Halogens'=>'52c2e9','Metals'=>'adaeac','Transactinides'=>'3d80be','Lanthanides'=>'4e68ab','Actinides'=>'a377ae','undefined'=>'ffffff'];}
...
foreach($r as $k=>$v)if($k){$i++; 
	...
	$x+=$w;}
	$ret.='[50,640,9§@Davy 2003-2021:text]';
return $ret;}
...
$ret='[black,,,12px:attr]';
$ret.='[spl|spitable2;call|p1='.$n.'|mode§['.($x+6).','.($y+15).',font-size:12px§'.$n.':text]:bj]';
...
list($name,$sym,$fam,$layer,$level)=val($r,$i,['','','','','']);
list($ring,$subring,$pos)=self::findpos($level,$i);
//list($ring,$subring,$pos)=self::findpos_layer($layer,$i);
list($x,$y,$w,$h,$t)=self::atompos($ring,$subring,$pos,$i,$sym,$mode);
...
//$j=$rid.'|spitable2,call|p2='.$o.'|inpspi';
//$ret=inputcall($j,'inpspi',$p,4);
//$ret.=bj($j,picto('ok',24),'btsav').' ';
$j=$rid.'|spitable2,call||lblbar,mode';
$ret=bar('bar',$p,1,1,120,'1',ajx($j),'','');
//if($p>0)$ret.=bj($rid.'|spitable2,call|p1='.($p-1).',p2='.$o,picto('previous'),'btn').'';
//if($p<self::$max)$ret.=bj($rid.'|spitable2,call|p1='.($p+1).',p2='.$o,picto('next'),'btn').' ';
$ra=[1=>'left','valences/left','right','valences/right'];//,'angular'
foreach($ra as $k=>$v)$ret.=bj(self::$cb.'|spitable2,call|p1='.$p.',p2='.$k,$v,'btn').'';
$ret.=hlpbt('spitable');
...
add_head('jscode',self::injectJs($rid.'|spitable2,call',$a,$o));
$ret=self::call($p);
