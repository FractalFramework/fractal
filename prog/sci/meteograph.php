...
//list($rn2,$qv2)=self::mediane($r,2);//median
list($rn1,$qv1)=self::mediane($r,4);//first quartile
list($rn3,$qv3)=self::mediane($r,4/3);//last quartile
...
//else $rc=$rb;
ksort($rc); //pr($rc);
$f='usr/'.ses('user').'/weather/'.$m;
if($rc)$ret=db::save($f,$rc).self::play($rc,$m);
$fb='usr/'.ses('user').'/graphics/7';
